...
object main {
  ...
  class hash_function(numBuckets_in: Long) extends Serializable {
    val p: Long = 2147483587  // prime number near 2^31
    val a: Long = (rand.nextLong % (p - 1)) + 1  // random in [1, p)
    val b: Long = (rand.nextLong % p)             // random in [0, p)
  ...
  // 4-universal Rademacher hash function returning +1 or -1 with equal probability
  ...
  // Helper: perform one trial of the BJKST sketch
  ...
  // BJKST: run multiple independent trials and return the median estimate.
  ...
  def Tug_of_War(x: RDD[String], width: Int, depth: Int): Long = {
    ...
    val sketchSums: Array[Long] = x.aggregate(zero)(
      ...
      (acc1, acc2) => {
        for (i <- 0 until totalSketches) { acc1(i) += acc2(i) }
        acc1
      }
    )
  ...
  def exact_F2(x: RDD[String]): Long = {
    val ans = x.map(i => (i, 1L)).reduceByKey(_ + _)
  ...
  def main(args: Array[String]): Unit = {
    ...
    args(1) match {
      case "BJKST" =>
        ...
        println("==================================")
        println("BJKST Algorithm. Bucket Size: " + args(2) + ". Trials: " + args(3) +
      ...
      case "ToW" =>
        ...
        val durationSeconds = (endTimeMillis - startTimeMillis) / 1000
        println("==================================")
        println("Tug-of-War F2 Approximation. Width: " + args(2) + ". Depth: " + args(3) +
          ". Time elapsed: " + durationSeconds + " s. Estimate: " + ans)
        println("==================================")
      case "exactF2" =>
        if (args.length != 2) {
